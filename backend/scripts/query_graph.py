...
def format_node(row: Dict[str, Any]) -> dict:
    ...
    return {
        ...
        "position_absolute": {
            "x": float(row['position_absolute_x']) if row['position_absolute_x'] is not None else float(row['position_x']),
            "y": float(row['position_absolute_y']) if row['position_absolute_y'] is not None else float(row['position_y']),
        },
    ...
    try:
        with engine.connect() as conn:
            ...
            graph_result = conn.execute(
                text("SELECT * FROM graphs WHERE id = :graph_id"),
                {"graph_id": graph_uuid}
            )
            ...
            if nodes:
                for idx, node_row in enumerate(nodes, 1):
                    ...
                    print(f"  数据: {data_preview}{'...' if len(json.dumps(node_info['data'], ensure_ascii=False)) > 200 else ''}")
                    print()
            else:
                print("  无节点")
            print()
            ...
            if edges:
                for idx, edge_row in enumerate(edges, 1):
                    ...
                    print(f"  数据: {data_preview}{'...' if len(json.dumps(edge_info['data'], ensure_ascii=False)) > 200 else ''}")
                    print()
            else:
                print("  无边")
            print()
