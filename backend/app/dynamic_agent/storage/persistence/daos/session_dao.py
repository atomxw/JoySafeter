        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt >= max_retries - 1:
                    logger.error(
                        f"DB connection error after {max_retries} attempts", exc_info=e
                    )
                    break
    ...
    async def add_message(self, session_id: str, role: str, content: str, metadata: Optional[Dict[str, Any]] = None) -> int:
        ...
        async def _do_insert():
            async with self.pool.acquire(timeout=5.0) as conn:
                ...
                """, session_id, role, content, json.dumps(serializable_metadata))
                return result
    ...
    async def load_context(self, session_id: str) -> Optional[SessionContext]:
        ...
        async def _do_load():
            async with self.pool.acquire(timeout=5.0) as conn:
                ...
                completed_tasks = json.loads(meta_row['completed_tasks']) if meta_row and meta_row['completed_tasks'] else []
        ...
        async def _do_list():
            async with self.pool.acquire(timeout=5.0) as conn:
                total = await conn.fetchval(
                    "SELECT COUNT(*) FROM session_contexts WHERE user_id = $1",
                    user_id
                )
