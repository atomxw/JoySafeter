...
class FileTools(Toolkit):
    ...
    def save_file(self, contents: str, file_name: str, overwrite: bool = True, encoding: str = "utf-8") -> str:
        ...
        try:
            ...
            if not (safe):
                logger.error(f"Attempted to save file: {file_name}")
    ...
    def read_file_chunk(self, file_name: str, start_line: int, end_line: int, encoding: str = "utf-8") -> str:
        ...
        try:
            ...
            safe, file_path = self.check_escape(file_name)
            if not (safe):
                logger.error(f"Attempted to read file: {file_name}")
    ...
    def replace_file_chunk(
        ...
        """
        try:
            logger.debug(f"Patching file: {file_name}")
            safe, file_path = self.check_escape(file_name)
            if not (safe):
                logger.error(f"Attempted to read file: {file_name}")
                return "Error reading file"
    ...
    def delete_file(self, file_name: str) -> str:
        ...
        try:
            if safe:
                ...
                logger.error(f"Attempt to delete file outside {self.base_dir}: {file_name}")
    ...
    def check_escape(self, relative_path: str) -> Tuple[bool, Path]:
        ...
        try:
            ...
            logger.error("Attempted to escape base_dir")
    ...
    def list_files(self, **kwargs) -> str:
        ...
        try:
            logger.debug(f"Reading files in : {self.base_dir}/{directory}")
    ...
    def search_files(self, pattern: str) -> str:
        ...
        try:
            ...
            logger.debug(f"Searching files in {self.base_dir} with pattern {pattern}")
            ...
            logger.debug(f"Found {len(file_paths)} files matching pattern {pattern}")
